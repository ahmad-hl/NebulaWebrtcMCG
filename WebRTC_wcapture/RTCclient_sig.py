...
async def run(pc, recorder, signaling, servsocket, disp_logger, rtt_logger, bw_logger):
    ...
    async def send_pings():
        while True:
            key = "p"
            # key = getch.getch()
            print("channel(%s) %s" % (channel.label, "ping %d" % current_stamp()))
    ...
    def on_message(message):
        ...
        if isinstance(message, str) and message.startswith("pong"):
            ...
            rtt_logger.info("{},{}".format(round(time.time()),elapsed_ms))
...
if __name__ == "__main__":
    ...
    rtt_log_url = os.path.join(Log_PATH, "rtt.rtc.log")
    bw_log_url = os.path.join(Log_PATH, "bw.rtc.log")
    ...
    disp_logger.info("frame_no,delay,timebase,pts")
    ...
    bw_logger.info("seconds,frame_no,bw")
    ...
    try:
        ...
        loop.run_until_complete( run(pc=pc, recorder=recorder,
                signaling=signaling, servsocket=sock, disp_logger=disp_logger, rtt_logger=rtt_logger, bw_logger=bw_logger))
